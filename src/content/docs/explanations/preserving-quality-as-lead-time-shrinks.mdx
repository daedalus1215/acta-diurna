...
Recently, we shipped twice in a single day: once earlier than usual to meet delivery deadlines and begin UAT (which due to external constraints—occurs in production). In doing so, we skipped a full manual regression pass and introduced a defect into PROD.
