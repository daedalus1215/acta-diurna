...
Metrics such as deployment frequency, lead time for changes, change failure rate, and mean time to restore are often misunderstood as a mandate to move faster at the expense of safety. In practice, they are more useful as a way to understand where a delivery system stops scaling.
...
However, that balance changes when lead time expectations collapse from days to minutes — as they do during production incidents or urgent customer-impacting defects.

Recently, we shipped twice in a single day: once earlier than usual to meet delivery deadlines and begin UAT, which—due to external constraints—occurs in production. In doing so, we proceeded without a full manual regression pass and introduced a defect into PROD.
...
The system held — but not because it was designed to operate safely at that speed. It held because people adapted to constraints in the moment. That adaptability is valuable, but it is not a sustainable strategy for maintaining quality as lead time expectations shrink.
